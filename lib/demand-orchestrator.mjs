 ...
 */
import { classify, parseGithub, NEEDS_BY_INTENT, norm, descriptionIntentScore } from './router.mjs';
...
function checkPedidoAlinhado(demand, snapshot, plan) {
  ...
  if (folder && FERRAMENTAS_IDS.has(folder)) {
    push(
      checks,
      'pasta_produto',
      false,
      'Pasta é morador do condomínio, não app de produto',
      'Escolha um app em "Seus projetos" (FREEDOM, XAXA…), não ' + folder,
      'fail'
    );
  }

  if (desc && !gh && descriptionIntentScore(desc) === 0) {
    push(
      checks,
      'pedido_vago',
      false,
      'Descrição reconhecível pelo ecossistema',
      'Use termos como: nova funcionalidade, auditar, backup/SRE, projeto novo, FIRE, bug…',
      'fail'
    );
  }
...
function buildVerdict(alignScore, execScore, checks) {
  ...
  if (
    fails.some((c) =>
      ['intent_coerente', 'pedido_presente', 'pedido_vago', 'pasta_produto'].includes(c.id)
    )
  ) {
    verdict = 'desalinhado';
...
function buildRecommendations(verdict, alignChecks, execChecks, demand) {
  ...
  if (fail('pedido_vago')) {
    rec.push('Descreva com palavras do ecossistema: nova funcionalidade, auditar, backup, projeto novo, FIRE, correção…');
  }
  if (fail('pasta_produto')) {
    rec.push('Selecione um app de produto na lista — moradores (workbench, dLogica) entram no relatório, não como pasta de trabalho.');
  }
 ...
 */
function buildGates(verdict, allChecks, demand) {
  const blockers = allChecks.filter((c) => !c.ok && c.severity === 'fail');
  const warnings = allChecks.filter((c) => !c.ok && c.severity === 'warn');
  const status = demand.status || 'draft';
  const gateFails = allChecks.filter(
    (c) => !c.ok && (c.id.startsWith('gate_') || c.id === 'intent_coerente' || c.id === 'pedido_presente')
  );

  return {
    allow_save_demand: verdict !== 'desalinhado',
    allow_status_completed: verdict !== 'desalinhado' && !gateFails.some((c) => c.id.startsWith('gate_')),
    allow_skip_sequence: false,
    blockers: blockers.map((c) => ({ id: c.id, label: c.label, detail: c.detail })),
    warnings: warnings.map((c) => ({ id: c.id, label: c.label, detail: c.detail })),
    architecture_ok: !blockers.some((c) =>
      ['intent_coerente', 'morador_principal', 'pedido_presente', 'plano_needs'].includes(c.id)
    ),
    execution_ok:
      verdict === 'adequado' ||
      (verdict === 'plano_ok' && status === 'draft') ||
      !blockers.some((c) => c.id.startsWith('gate_'))
  };
}

/** Impede salvar demanda com pedido incoerente (arquitetura de roteamento errada). */
export function validateDemandSave(analyzed, force = false) {
  const orchestration = orchestrateAnalyzed(analyzed);
  const gates = buildGates(
    orchestration.verdict,
    [...orchestration.alignment.checks, ...orchestration.execution.checks],
    analyzed.demand
  );
  orchestration.gates = gates;
  if (!force && !gates.allow_save_demand) {
    return {
      ok: false,
      code: 'ORCHESTRATION_BLOCKED',
      message:
        'Demanda bloqueada: pedido desalinhado ao propósito (intent). Corrija a descrição antes de salvar.',
      orchestration
    };
  }
...
/** Impede status completed sem contratos dos moradores. */
export function validateStatusPatch(record, newStatus, force = false) {
  const orchestration = orchestrateRecord(record);
  const allChecks = [...orchestration.alignment.checks, ...orchestration.execution.checks];
  const gates = buildGates(orchestration.verdict, allChecks, record.demand);
  orchestration.gates = gates;

  if (!force && newStatus === 'completed' && !gates.allow_status_completed) {
    const failed = allChecks.filter((c) => !c.ok && c.id.startsWith('gate_'));
    return {
      ok: false,
      code: 'ORCHESTRATION_BLOCKED',
      message:
        'Status "completed" bloqueado: execução incompleta (gates workbench/Cursor/Max). Conclua os moradores com output_payload.',
      orchestration,
      failed_gates: failed.map((c) => c.label)
    };
  }
  return { ok: true, orchestration };
}

/** Ordem das 4 portas: não pular morador anterior pendente. */
export function validateRunComplete(record, runKey) {
  const runs = (record.runs || [])
    .filter((r) => r.resident && r.resident !== 'ecomaestro')
    .sort((a, b) => (a.sequence_order || 0) - (b.sequence_order || 0));
  const target = runs.find((r) => r.id === runKey || r.resident === runKey);
  if (!target) return { ok: false, code: 'RUN_NOT_FOUND', message: 'Passagem não encontrada' };
  const idx = runs.indexOf(target);
  for (let i = 0; i < idx; i++) {
    const prev = runs[i];
    if (prev.status !== 'done' && prev.status !== 'skipped') {
      return {
        ...
        message:
          'Execução fora de ordem: conclua primeiro "' +
          (prev.resident || prev.id) +
          '" antes de "' +
          (target.resident || target.id) +
          '".'
      };
    }
  }
...
export function orchestrateRecord(record) {
  ...
  const gates = buildGates(verdict, allChecks, demand);

  return {
    orchestrator: 'ecomaestro-adequacao-v2',
