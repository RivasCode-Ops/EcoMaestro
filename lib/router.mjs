 ...
 */
import { buildProjectSetup, PROJETOS_ROOT } from './project-setup.mjs';
import { buildCursorKit, shouldShowCursorKit } from './cursor-kit.mjs';
import { detectEcoOverlaps } from './eco-overlaps.mjs';
import { toEcoWebHref } from './eco-href.mjs';
import { verifyAplicadores, verifyOverlapLinks, verifyKitLinks } from './verify-links.mjs';
...
export const CATALOGO = [
  ...
  { keys: ['prompt method', 'prompt hub'], id: 'prompt_legacy', pasta: 'PROMPT' },
...
export const APPS = {
  ...
  simMoto: {
    ...
    href: '../Simulador-Troca-Moto/README.md',
    order: 14,
    tipo: 'complemento',
    resident: 'simulador_troca_moto'
  },
  ecomaestro: {
...
export const NEEDS_BY_INTENT = {
  ...
  infra_resiliencia: [
    'Contexto técnico (banco, SO, Docker, ambiente prod)',
    'Estratégia de backup + isolamento + retenção no workbench (04-INFRAESTRUTURA)',
    'Scripts de dump/cron e envio S3 no Cursor (sem senhas no código)',
    'Monitoramento (webhook) e roteiro de restore testado',
    'Auditoria Max no repo (CI, secrets, .env.example)'
  ],
  ...
  coding_dev: [
    'D00 — contexto da sessão (workbench / 04-coding-diario)',
    'Prompt D01 (feature) ou D02 (bug) colado no Cursor',
    'Rules .cursor/rules no repo (modelo GeoGrowth-Cursor)',
    'Max após mudanças relevantes no código'
  ]
};
...
const KW = [
  ...
  {
    keys: [
      'cursor rules', 'regras cursor', '.cursor', 'coding diario', 'coding diário',
      'codar', 'implementar no cursor', 'composer', 'agent rules', 'regra cursor'
    ],
    intent: 'coding_dev',
    tipo: 'feature',
    coding: true
  ...
  {
    keys: [
      'backup', 'backups', 'disaster recovery', 'recuperacao de desastre', 'recuperação de desastre',
      'sre', 'resiliencia', 'resiliência', 'dump', 'cron job', 'aws s3', 'restore', 'postgres dump',
      'criptografar backup', 'webhook slack', 'lifecycle', 'point-in-time', 'pitr'
    ],
    intent: 'infra_resiliencia',
    tipo: 'feature',
    wb: 'Projeto Novo'
  },
...
/** Força mínima da descrição para o orquestrador (0 = vaga). */
export function descriptionIntentScore(desc) {
  const t = norm(desc);
  if (t.length < 10) return 0;
  let score = 0;
  for (const rule of KW) {
    for (const k of rule.keys) {
      if (t.includes(norm(k))) score += 2;
    }
  }
  return score;
}
...
export function classify(desc, gh, forcedIntent = null) {
  ...
  let intent = forcedIntent || (best.rule ? best.rule.intent : null);
  ...
  if (intent === 'infra_resiliencia') {
    return {
      intent,
      tipo,
      moradores: ['dlogica', 'workbench', 'cursor', 'max'],
      extras: ['cortana', ...extras],
      primary: 'workbench',
      wb: 'Projeto Novo',
      cat,
      routing_label: 'Auditoria SRE / backup / DR'
    };
  }
  ...
  if (intent === 'coding_dev') {
    return {
      intent,
      tipo,
      moradores: ['workbench', 'cursor', 'max'],
      extras,
      primary: 'workbench',
      wb: '20-ENTREGA-DE-PRODUTO',
      coding: true,
      cat,
      routing_label: 'Coding / rules no Cursor'
    };
  }
  if (intent === 'feature_nova') {
    return {
      ...
      coding: true,
      cat,
      routing_label: 'Detectado pela descrição'
    };
  }
...
export function buildAplicadores(plan, gh, cat, projectFolder = null) {
  const list = [];
  const push = (id, principal) => {
    ...
    if (id === 'workbench' && plan.coding) {
      href = '../workbench/20-ENTREGA-DE-PRODUTO/04-coding-diario/README.md';
    } else if (id === 'workbench' && plan.wb === 'Projeto Novo') {
      href = '../workbench/Projeto Novo/04-INFRAESTRUTURA.md';
    } else if (id === 'workbench' && plan.wb) {
      href = '../workbench/' + plan.wb + '/README.md';
    }
    let note = app.faz;
    if (id === 'workbench' && plan.intent === 'infra_resiliencia') {
      note = 'Kit 04-INFRAESTRUTURA + D11 incidente/DR — backup, cron, S3, restore e secrets em .env.';
    } else if (id === 'workbench' && plan.coding) {
      note = 'Handoff + prompts D00–D12 (kit 04-coding-diario). Passo 2 das 4 portas.';
    }
    if (id === 'cursor') {
      href = '../workbench/Cursor-Kit/README.md';
      const pasta = (cat && cat.pasta) || projectFolder;
      note =
        'Abra no Cursor a pasta do app (não só o workbench). D00 → D01 ou D02 no 04-coding-diario.';
      if (pasta) note += ' Pasta obrigatória: c:\\_PROJETOS\\' + pasta;
      if (gh) note += ' Repo: ' + gh.org + '/' + gh.repo;
    }
...
export function analyzeDemand({
  ...
  const folder = project_folder && project_folder !== '__new__' ? project_folder : null;

  if (!link && !desc && !folder) {
    const err = new Error('Escolha um projeto em _PROJETOS e/ou descreva a demanda');
    err.code = 'VALIDATION';
    throw err;
  }

  if (folder && desc && !desc.includes(folder)) {
    desc = '[' + folder + '] ' + desc;
  }

  const gh = parseGithub(link);
  const catFromFolder = folder && !gh ? matchCatalog(folder, null) : null;
  const plan = classify(desc, gh, _force_intent);
  const cat = plan.cat || catFromFolder || matchCatalog(desc, gh);
  ...
  const aplicadores = buildAplicadores(plan, gh, cat, folder);
  const setup = buildProjectSetup(plan, gh, desc);
  let cursor_kit = shouldShowCursorKit(plan, desc) ? buildCursorKit(plan) : null;
  if (cursor_kit?.links) {
    cursor_kit = {
      ...cursor_kit,
      links: verifyKitLinks(cursor_kit.links.map((l) => ({ ...l, href: toEcoWebHref(l.href) })))
    };
  }
  let setupResolved = setup;
  if (setup) {
    setupResolved = {
      ...setup,
      tutorial_href: toEcoWebHref(setup.tutorial_href),
      orquestrador_href: toEcoWebHref(setup.orquestrador_href),
      bat_href: setup.bat_href
    };
  }
  let eco_overlaps = detectEcoOverlaps(desc, link);
  if (eco_overlaps) {
    eco_overlaps = verifyOverlapLinks(
      eco_overlaps.map((o) => ({ ...o, href: o.href ? toEcoWebHref(o.href) : null }))
    );
  }
  const conf = confidencePct(plan, desc, gh);
  if (setup) {
    needs.unshift('Criar pasta local em c:\\_PROJETOS\\' + setup.suggested_slug + ' (botão abaixo)');
  }

  const source_type = gh && desc ? 'mixed' : gh ? 'github' : folder ? 'localhost' : 'description';
  ...
  const payload_snapshot = {
    ...
    context: {
      ...
      localhost_url: folder_path || (folder ? PROJETOS_ROOT + '\\' + folder : null),
      project_folder: folder,
      raw_description: desc,
      tipo_demanda: plan.tipo,
      suggested_slug: setup?.suggested_slug || null,
      projetos_folder: setup?.folder_path || null
    },
  ...
  return {
    demand: {
      ...
      github_url: gh ? gh.url : null,
      project_folder: folder,
      local_folder: folder_path || (folder ? PROJETOS_ROOT + '\\' + folder : null),
      tipo_demanda: plan.tipo,
      status: 'draft',
    ...
    report: {
      ...
      cursor_kit,
      eco_overlaps,
      aplicadores: verifyAplicadores(
        aplicadores.map((a) => ({
          id: a.id,
          name: a.name,
          label: a.label,
          note: a.note,
          href: a.href ? toEcoWebHref(a.href) : null,
